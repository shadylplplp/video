...
@section('content')
    ...
    <div style="margin: auto; width: 70%; height: 500px;">
        <ul class="nav nav-pills nav-stacked" style="width:150px; float: left;position: relative;">
            ...
            <li role="presentation" class="active"><a href="{{route('user.myfans')}}">我的粉丝</a></li>
        ...
        <div style="position: relative; margin-left: 15px; height: 600px; width: 80%; background: #0d8ddb;float: left; background-color: transparent;box-shadow:0px 0px 8px 3px #ccc;">
            <table class="table" style="margin-left: 30px; margin-top: 10px; width:800px;">
                ...
                <tr>
                    <td>
                        <a href="{{route('user.showusercennter',$follow->FollowUser->id)}}"><img src="{{$follow->FollowUser->avatar}}" style="width: 40px; height: 40px; margin-left: 5px;"></a>
                        <a href="{{route('user.showusercennter',$follow->FollowUser->id)}}"><span style="margin-left: 5px;">{{$follow->FollowUser->name}}</span></a>
                    </td>
                </tr>
                @endforeach
                <tr>
